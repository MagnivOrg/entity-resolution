...
def nightly_task(): 
    new_affiliations = session.query(affiliations_table).filter_by(merged=False).limit(10).all()
    for affiliation in new_affiliations:
        # loop over all affiliations
        highest_cos_sim = 0
        highest_id = -1
        completed_affiliations = session.query(affiliations_table).filter_by(merged=True).all()
        for c_affiliation in completed_affiliations:
            cos_sim = util.cos_sim(np.array(json.loads(affiliation.embedding)), np.array(json.loads(c_affiliation.embedding))).item()
        ...
        if highest_cos_sim > 0:
            # delete the current
            highest_match = session.query(affiliations_table).filter_by(id=highest_id).first()
